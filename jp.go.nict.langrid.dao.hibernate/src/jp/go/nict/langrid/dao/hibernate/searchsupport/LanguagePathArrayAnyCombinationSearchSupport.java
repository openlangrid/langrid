 ...
 */
public class LanguagePathArrayAnyCombinationSearchSupport
...
implements SearchSupport<T>{
	public void filterResults(MatchingCondition condition, List<T> results) {
		...
		Iterator<T> i = results.listIterator();
		while(i.hasNext()){
			T s = i.next();
	...
	static boolean isMatched(LanguagePath[] conditionPaths, LanguagePath[] servicePaths){
		for(LanguagePath cp : conditionPaths){
			for(LanguagePath dp : servicePaths){
				if(LanguagePathMatcher.matchesInCombination(cp, dp)) return true;
			}
		}
		return false;
	}
	...
	private static Logger logger = Logger.getLogger(
			LanguagePathArrayAnyCombinationSearchSupport.class.getName());
}
