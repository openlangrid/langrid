...
import jp.go.nict.langrid.composite.backtranslation.BackTranslation;
...
public class BestTranslationSelectionForToolbox
...
implements TranslationService{
	...
	public String translate(String sourceLang, String targetLang,
		...
		for(int i = 1; i <= 5; i++){
			final TranslationService forwardTrans = csf.getService(
					"ForwardTranslationPL" + i, TranslationService.class);
			final TranslationService backwardTrans = csf.getService(
					"BackwardTranslationPL" + i, TranslationService.class);
			BackTranslationService backTrans = new BackTranslation(){
				@Override
					...
					return new ComponentServiceFactory() {
						...
						public <T> T getService(String invocationName, Class<T> interfaceClass) {
							if(invocationName.equals("ForwardTranslationPL")){
								return interfaceClass.cast(forwardTrans);
							} else if(invocationName.equals("BackwardTranslationPL")){
								return interfaceClass.cast(backwardTrans);
							}
							return null;
						}
					};
			...
			if(forwardTrans == null || backwardTrans == null || simiCalc == null){
				continue;
			}
