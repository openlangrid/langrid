 ...
 */
public class StreamingServiceSupervisor
implements Filter{
	@Override
	...
	throws ConnectException, DaoException, IOException, ServletException{
		...
		if(protocol.equals(Protocols.PROTOBUF_RPC)){
			responder = new HttpFaultResponder(request.getLocalName(), serviceGridId, serviceId);
		} else if(protocol.equals(Protocols.JSON_RPC)){
			responder = new HttpFaultResponder(request.getLocalName(), serviceGridId, serviceId);
		} else{
			responder = new SoapFaultResponder(request.getLocalName(), serviceGridId, serviceId);
		}
		...
		if(expInPreprocess != null){
			ByteArrayOutputHttpServletResponseWrapper expResponse
				= new ByteArrayOutputHttpServletResponseWrapper(response);
			...
			if(c != null && c.getTargetService() != null){
				...
				try{
					...
					LogInfo li = FrontEnd.createLogInfo(
		...
		if(!isStreaming){
			ByteArrayOutputHttpServletResponseWrapper wresponse
				= new ByteArrayOutputHttpServletResponseWrapper(response);
			...
			if(expInProcess == null){
				...
				ByteArrayOutputHttpServletResponseWrapper wresponse2
						= new ByteArrayOutputHttpServletResponseWrapper(response);
			...
			PostprocessingHttpServletResponseWrapper wresponse
					= new PostprocessingHttpServletResponseWrapper(
							response, c, fe
							, serviceContext.getSelfGridId().equals(c.getTargetGrid().getGridId())
					);
			try{
				long s = System.currentTimeMillis();
				// invoke
		...
		try{
			...
			if(code != 200){
				soapFault = SOAPBodyUtil.extractSoapFaultString(
						new ByteArrayInputStream(responseHead));
				if(soapFault.getFirst() == null && soapFault.getSecond() == null){
					// protobuf error?
					soapFault = Pair.create(
							"Server.unknownError"
							, StringEscapeUtils.escapeHtml(new String(
									responseHead, 0, Math.min(700, responseHead.length)
									, CharsetUtil.UTF_8
									))
							);
