/*
 * $Id: TemporaryUserAuthenticator.java 307 2010-12-01 06:05:27Z t-nakaguchi $
 *
 ...
 */
public class TemporaryUserAuthenticator extends AbstractLangridBasicAuthenticator{
	@Override
	...
	throws UserNotFoundException, DaoException
	{
		...
		String userId = getTemporaryUserDao().getParentUserIdIfUserAvailable(
				selfGridId, authUser, authDigestedPass);
		if(userId != null){
			context.setAuthorized(selfGridId, userId, getUserDao().getUser(
					selfGridId, userId).getPassword());
			return true;
		} else{
			return false;
		}
