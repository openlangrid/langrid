...
public class SoapClientFactoryLanguageServiceTest{
	@Test
...
/*/
		final OutputStream os = new FileOutputStream("MorphologicalAnalysisService.res");
		try{
			SoapClientFactory.setInputStreamFilter(new InputStreamFilter() {
				@Override
				public InputStream filter(InputStream is) {
					return new DuplicatingInputStream(is, os);
				}
			});
			assertFieldsNotNull(create(MorphologicalAnalysisService.class).analyze("en", "hello world"));
		} finally{
...
//*/
	...
	private <T> T create(Class<T> intf) throws MalformedURLException, FileNotFoundException{
		if(intercept){
			...
			interceptedReq = Intercept.requestXmlToFile(simpleName + "req.xml");
			interceptedRes = Intercept.responseXmlToFile(simpleName + "res.xml");
		}
