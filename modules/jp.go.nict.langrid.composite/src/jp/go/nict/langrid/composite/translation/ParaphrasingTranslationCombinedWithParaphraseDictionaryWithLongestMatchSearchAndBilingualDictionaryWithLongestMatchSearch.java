 ...
 */
public class ParaphrasingTranslationCombinedWithParaphraseDictionaryWithLongestMatchSearchAndBilingualDictionaryWithLongestMatchSearch
...
implements TranslationWithTemporalDictionaryService{
	...
	UnsupportedLanguagePairException
	{
		...
		try{
			...
			ReplacementTerm rt = new ReplacementTerm(RIProcessor.getCurrentServiceContext());
			...
			TranslationWithPosition[] paraDictResult = new TranslationWithPosition[]{};
			{
				log("prepare for ParaphraseDictionaryWithLongestMatchSearchService.searchLongestMatchingTerms()");
				Morpheme[] m = analyzeResult;
				log("invoke ParaphraseDictionaryWithLongestMatchSearchService.searchLongestMatchingTerms(" +
						m.length + "morphs["
						...
						+ "])");
				try{
					paraDictResult = factory.getService(
							"ParaphraseDictionaryWithLongestMatchSearchPL"
				...
				log("invoke ParaphraseDictionaryWithLongestMatchSearchService.searchLongestMatchingTerms done(" +
						paraDictResult.length + "translations)");
			}

			analyzeResult = merger.merge(analyzeResult, paraDictResult);
			...
			SourceAndMorphemesAndCodes joinedSmc = csmc.doConstructSMC(
					sl, analyzeResult, dictResult);
			...
			String fwTranslationResult = factory.getService(
					"TranslationPL", TranslationService.class
					).translate(sl, tl, joinedSmc.getSource());
