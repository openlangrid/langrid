/*
 * $Id: AllLanguageServicesPage.java 406 2011-08-25 02:12:29Z t-nakaguchi $
 * 
...
import jp.go.nict.langrid.management.web.view.page.language.service.admin.component.list.row.YourLanguageServicesListRowAdminPanel;
import jp.go.nict.langrid.management.web.view.page.language.service.atomic.admin.RegistrationOfAtomicServiceUserListPage;
import jp.go.nict.langrid.management.web.view.page.language.service.component.list.ServiceListPanel;
import jp.go.nict.langrid.management.web.view.page.language.service.composite.admin.RegistrationOfCompositeServiceUserListPage;
 ...
 */
public class AllLanguageServicesPage extends YourLanguageServicesPage{
	public AllLanguageServicesPage(){
		addOrReplace(new Label("headline", MessageManager.getMessage(
				"LanguageGridOperator.headline.AllLanguageServices", getLocale())).setRenderBodyOnly(true));
		addOrReplace(new Link("atomicRegister"){
			@Override
			public void onClick(){
				setResponsePage(new RegistrationOfAtomicServiceUserListPage());
			}

			private static final long serialVersionUID = 1L;
		});
		addOrReplace(new Link("compositeRegister"){
			@Override
			public void onClick(){
	...
	throws ServiceManagerException
	{
		...
		ServiceListPanel<T> list = new ServiceListPanel<T>(getSelfGridId(), listId, provider){
			...
			throws ServiceManagerException
			{
				return new YourLanguageServicesListRowAdminPanel(
					...
					protected void doCancelProcess(String serviceId){
						try{
						   String gridId = getSelfGridId();
							...
							setResponsePage(new UnregistrationOfLanguageServicesCancelResultAdminPage(serviceId));
...
//                     factory.getLangridServiceService(gridId).deleteById(serviceId);
                     factory.getOperationService(gridId, gridId, "").deleteByTargetId(serviceId, OperationType.SERVICE);
                     setResponsePage(new UnregistrationOfLanguageServiceNowResultAdminPage(model));
                  } catch(ServiceManagerException e) {
                     doErrorProcess(e);
                  }
               }
