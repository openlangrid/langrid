 ...
 */
public class TranslationWithBackup
extends AbstractCompositeService
implements TranslationService{
	public static class Services{
		@Invocation(name="TranslationPL", required=true)
		private TranslationService trans;
		@Invocation(name="BackupTranslationPL")
		private TranslationService backupTrans;
	}

	public TranslationWithBackup() {
	...
	UnsupportedLanguagePairException
	{
		try{
			...
			boolean done = false;
			try{
				String ret = s.trans.translate(sourceLang, targetLang, source);
				done = true;
				return ret;
			} finally{
				if(!done && s.backupTrans != null){
					return s.backupTrans.translate(sourceLang, targetLang, source);
				}
			}
