...
public class EditPage extends ServiceManagerPage{
	 ...
	 * @param model
	 */
	public EditPage(final DomainModel model){
		AbstractForm<String> form = new AbstractForm<String>("form", getSelfGridId()){
			...
			protected void addComponents(String initialParameter) throws ServiceManagerException{
				add(new Label("id", new Model<String>(model.getDomainId())));
				add(nameField = new NameTextField("name", model.getDomainName(), true));
				...
				add(new Button("edit") {
					...
					public void onSubmit() {
						try {
							model.setDomainName(nameField.getValue());
							model.setDescription(descriptionField.getValue());
							ServiceFactory.getInstance().getDomainService(getSelfGridId()).edit(model);
			...
			protected void setResultPage(String resultParameter){
			   setResponsePage(new EditResultPage(model));
			}
